...
# proper input format:
# iridium-parser.py --filter IridiumRAMessage --format ra_sat,ra_cell,ra_pos_x,ra_pos_y,ra_pos_z,globaltime
...
for cnt in range(len(xs)):
    ...
    p=plt.scatter(x=xs[cnt], y=ys[cnt], c=colorst[cnt], edgecolor="none",label="%02d (%d)"%(cnt,len(xs[cnt])) )
