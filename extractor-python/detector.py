            ...
            while True:
                ...
                if index%self._search_size==0:
                    ...
                    if len(fft_hist)>25: # grace period after start of file
                        ...
                        if self._verbose:
                            for p in peaks:
                                print("[%4d,%2d,%2d]"%(p[0],p[1],index-p[2]), end=' ')
                        for p in peaks:
                            pi=p[0]
                            if self._verbose:
                                print("Peak B%4d: %4.1f dB"%(pi,10*math.log(peakl[pi],10)), end=' ')
                                pa=numpy.average(peakl[pi-10:pi+10])
                                print("(avg: %4.1f dB)"%(10*math.log(pa,10)), end=' ')
                            if peakl[p[0]]>self._fft_peak:
                                if self._verbose:
                                    print("still peak", end=' ')
                    ...
                    if burst_signals==self._burst_size:
                        ...
                        burst_signals=0
                        time_stamp = index*self._bin_size
                        print("Ran into burst squelch at", time_stamp, file=sys.stderr)

                    peaks_to_collect = [e for e in peaks if e[1]<=0]
                    for peak in peaks_to_collect:
                        data_collector(peak[3][0], peak[3][1], peak[3][2], peak[3][3], peak[4])
                    peaks = [e for e in peaks if e[1]>0]
